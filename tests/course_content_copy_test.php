...
try {
    ...
    $exam = $admin->query("SELECT status,scheduled_start_at_utc FROM timed_exams WHERE course_id='destination' AND item_id='" . $admin->real_escape_string($section['item_ids'][2]) . "'")->fetch_assoc();
    $assert(($exam['status'] ?? '') === 'draft' && ($exam['scheduled_start_at_utc'] ?? null) === null, 'Copied Timed Exam was not safely drafted.');
