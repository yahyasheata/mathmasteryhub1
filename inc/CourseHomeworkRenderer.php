...
if (!function_exists('mmh_homework_part')) {
    function mmh_homework_part(mysqli $conn, array $course, array $selection, array $resource, ?array $assignment, ?array $submission, $part)
    {
        ...
        if ($part === 'homework' || $part === 'homework-2') {
            $slotKey = $part === 'homework-2' ? 'homework_resource_2' : 'homework_resource';
            $slot = is_array($resource[$slotKey] ?? null) ? $resource[$slotKey] : ($part === 'homework' ? ['url' => $resource['homework_url'] ?? ''] : []);
            $url = mmh_course_resource_safe_url($slot['url'] ?? '');
            if ($url === null) return null;
            return ['url' => $url, 'resource_type' => $slot['resource_type'] ?? mmh_course_resource_type_for_url($url, 'external_link'), 'label' => $part === 'homework-2' ? 'Homework PDF 2' : 'Homework PDF 1', 'icon' => 'fas fa-file-alt'];
        }
...
if (!function_exists('mmh_homework_render')) {
    function mmh_homework_render(mysqli $conn, $baseUrl, $userId, array $course, array $selection, $itemId, array $resource, array $planContext = [], ?array $gatewayContext = null)
    {
        ...
        $homework2 = mmh_homework_part($conn, $course, $selection, $resource, $assignment, $submission, 'homework-2');
        ...
        $homeworkUrl2 = $homework2 ? course_resource_plan_url($baseUrl, $course['course_id'], $itemId, $planContext, (int) ($planContext['task']['id'] ?? 0)) . $partSeparator . 'part=homework-2' : '';
...
<section class="course-homework-resource-grid" aria-label="Homework resources"><?php if ($homeworkUrl): ?><article class="course-homework-panel course-homework-resource-card"><div class="course-homework-resource-heading"><span class="fas fa-file-alt" aria-hidden="true"></span><div><p class="course-resource-viewer-meta">Homework resource</p><h2>Homework PDF 1</h2></div></div><p>Open the homework file inside the LMS whenever the provider supports it.</p><a class="course-btn course-btn-primary" href="<?= mmh_homework_escape($homeworkUrl) ?>"><span class="fas fa-eye" aria-hidden="true"></span> View / Download PDF 1</a></article><?php endif ?><?php if ($homeworkUrl2): ?><article class="course-homework-panel course-homework-resource-card"><div class="course-homework-resource-heading"><span class="fas fa-file-alt" aria-hidden="true"></span><div><p class="course-resource-viewer-meta">Homework resource</p><h2>Homework PDF 2</h2></div></div><p>Additional homework material for this assignment.</p><a class="course-btn course-btn-primary" href="<?= mmh_homework_escape($homeworkUrl2) ?>"><span class="fas fa-eye" aria-hidden="true"></span> View / Download PDF 2</a></article><?php endif ?><?php if ($modelPolicyAllowed): ?><article class="course-homework-panel course-homework-resource-card<?= $modelRelease['available'] ? '' : ' is-locked' ?>"><div class="course-homework-resource-heading"><span class="fas fa-graduation-cap" aria-hidden="true"></span><div><p class="course-resource-viewer-meta">Model answer</p><h2>Model Answer</h2></div></div><?php if ($modelRelease['available'] && $modelUrl): ?><p>Use the model answer to review your method and final solution.</p><a class="course-btn course-btn-secondary" href="<?= mmh_homework_escape($modelUrl) ?>"><span class="fas fa-eye" aria-hidden="true"></span> View / Download Model Answer</a><?php else: ?><p class="course-homework-resource-state"><span class="fas fa-lock" aria-hidden="true"></span> <?= mmh_homework_escape($modelRelease['label']) ?></p><?php endif ?></article><?php endif ?></section>
