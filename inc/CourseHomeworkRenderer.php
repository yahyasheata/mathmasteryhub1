...
if (!function_exists('mmh_homework_part')) {
    function mmh_homework_part(mysqli $conn, array $course, array $selection, array $resource, ?array $assignment, ?array $submission, $part)
    {
        ...
        if ($part === 'homework') {
            ...
            if ($url === null) return null;
            return ['url' => $url, 'resource_type' => $slot['resource_type'] ?? mmh_course_resource_type_for_url($url, 'external_link'), 'label' => 'Homework resource', 'icon' => 'fas fa-file-alt'];
        }
...
if (!function_exists('mmh_homework_render')) {
    function mmh_homework_render(mysqli $conn, $baseUrl, $userId, array $course, array $selection, $itemId, array $resource, array $planContext = [], ?array $gatewayContext = null)
    {
        ...
        $modelPolicyAllowed = $model !== null && mmh_assignment_model_answer_access_can($conn, $assignmentId, (string) $course['course_id'], (int) $userId);
        if (!$modelPolicyAllowed) {
            $modelRelease = ['available' => false, 'label' => ''];
        }
        ...
        $maxSubmissionFiles = mmh_assignment_submission_max_files();
        $submittedAt = $submission && !empty($submission['submitted_at']) ? strtotime((string) $submission['submitted_at']) : false;
        $homeworkJsPath = __DIR__ . '/../resources/js/course-homework.js';
        $homeworkJsVersion = (string) (is_file($homeworkJsPath) ? (filemtime($homeworkJsPath) ?: 1) : 1);
...
<section class="course-homework-resource-grid" aria-label="Homework resources"><?php if ($homeworkUrl): ?><article class="course-homework-panel course-homework-resource-card"><div class="course-homework-resource-heading"><span class="fas fa-file-alt" aria-hidden="true"></span><div><p class="course-resource-viewer-meta">Homework resource</p><h2>Homework resource</h2></div></div><p>Open the homework file inside the LMS whenever the provider supports it.</p><a class="course-btn course-btn-primary" href="<?= mmh_homework_escape($homeworkUrl) ?>"><span class="fas fa-eye" aria-hidden="true"></span> View / Download homework</a></article><?php endif ?><?php if ($modelPolicyAllowed): ?><article class="course-homework-panel course-homework-resource-card<?= $modelRelease['available'] ? '' : ' is-locked' ?>"><div class="course-homework-resource-heading"><span class="fas fa-graduation-cap" aria-hidden="true"></span><div><p class="course-resource-viewer-meta">Model answer</p><h2>Model Answer</h2></div></div><?php if ($modelRelease['available'] && $modelUrl): ?><p>Use the model answer to review your method and final solution.</p><a class="course-btn course-btn-secondary" href="<?= mmh_homework_escape($modelUrl) ?>"><span class="fas fa-eye" aria-hidden="true"></span> View / Download Model Answer</a><?php else: ?><p class="course-homework-resource-state"><span class="fas fa-lock" aria-hidden="true"></span> <?= mmh_homework_escape($modelRelease['label']) ?></p><?php endif ?></article><?php endif ?></section>
<section class="course-homework-panel course-homework-submission"><p class="course-resource-viewer-meta">Submission</p><h2><?= mmh_homework_escape($status['label']) ?></h2><?php if ($submission): ?><?php $submissionFiles = mmh_assignment_submission_files($conn, $submission); ?><p><?php foreach ($submissionFiles as $fileIndex => $submissionFile): ?><?php if ($fileIndex): ?> · <?php endif ?><?php $submissionFileUrl = mmh_assignment_submission_file_url($baseUrl, $submissionFile['id'] ?? 0); ?><?php if ($submissionFileUrl): ?><a href="<?= mmh_homework_escape($submissionFileUrl) ?>" target="_blank" rel="noopener"><?= mmh_homework_escape(trim((string) ($submissionFile['original_filename'] ?? '')) !== '' ? (string) $submissionFile['original_filename'] : 'Submitted answer') ?></a><?php else: ?><span><?= mmh_homework_escape(trim((string) ($submissionFile['original_filename'] ?? '')) !== '' ? (string) $submissionFile['original_filename'] : 'Submitted answer') ?></span><?php endif ?><?php endforeach ?><?php if ($submittedAt): ?> · Submitted <?= mmh_homework_escape(date('j F Y \a\t g:i A',$submittedAt)) ?><?php endif ?><?php if (($submission['submission_source'] ?? '') === 'legacy_import'): ?> <span class="course-homework-imported-badge">Imported by Instructor</span><?php endif ?></p><?php if (trim((string)($submission['feedback'] ?? '')) !== ''): ?><p class="course-homework-feedback"><strong>Teacher feedback</strong><br><?= nl2br(mmh_homework_escape($submission['feedback'])) ?></p><?php endif ?><?php if (is_numeric($submission['grade'] ?? null)): ?><p class="course-homework-grade"><strong>Grade:</strong> <?= mmh_homework_escape($submission['grade']) ?></p><?php endif ?><?php else: ?><p>Upload your completed work before the deadline. You can replace it while submissions remain open.</p><?php endif ?><?php if ($uploadAllowed): ?><button class="course-btn course-btn-secondary" type="button" data-homework-upload><span class="fas fa-upload" aria-hidden="true"></span><?= $submission ? 'Replace submission' : 'Upload Homework' ?></button><?php endif ?></section>
<?php if ($uploadAllowed): ?><section class="course-homework-panel course-homework-upload" data-homework-upload-panel hidden><h2><?= $submission ? 'Replace submission' : 'Upload Homework' ?></h2><form data-homework-submission data-max-files="<?= mmh_homework_escape($maxSubmissionFiles) ?>" method="post" enctype="multipart/form-data" action="<?= mmh_homework_escape(mmh_homework_submission_endpoint($baseUrl)) ?>"><input type="hidden" name="assignment_id" value="<?= mmh_homework_escape($assignmentId) ?>"><input type="hidden" name="csrf_token" value="<?= mmh_homework_escape(student_course_csrf_token()) ?>"><label for="homework-files">Upload your answer <span>PDF, DOC, or DOCX · up to <?= mmh_homework_escape($maxSubmissionFiles) ?> files · one Homework submission</span></label><div class="course-homework-file-picker"><input id="homework-files" name="submission_files[]" type="file" accept=".pdf,.doc,.docx" multiple><p class="course-homework-file-picker-help"><span class="fas fa-file-pdf" aria-hidden="true"></span> Choose several PDFs together or add them one by one.</p></div><div class="course-homework-selected-files" data-homework-file-list hidden></div><?php if (!empty($assignment['allow_self_score'])): ?><label for="homework-score">My score<?php if (is_numeric($assignment['max_score'] ?? null)): ?> out of <?= mmh_homework_escape($assignment['max_score']) ?><?php endif ?></label><input id="homework-score" name="self_score" type="number" min="0" step="0.5"<?php if (is_numeric($assignment['max_score'] ?? null)): ?> max="<?= mmh_homework_escape($assignment['max_score']) ?>"<?php endif ?> required><?php endif ?><div class="course-homework-upload-actions"><button class="course-btn course-btn-primary" type="submit">Submit Homework</button><button class="course-btn course-btn-ghost" type="button" data-homework-upload-cancel>Cancel</button></div><p data-homework-upload-message role="status" aria-live="polite"></p></form></section><?php endif ?>
<nav class="course-resource-viewer-navigation" aria-label="Lesson navigation"><div><?php if ($previousUrl): ?><a class="course-resource-viewer-nav-link" href="<?= mmh_homework_escape($previousUrl) ?>"><span class="fas fa-arrow-left" aria-hidden="true"></span> Previous resource</a><?php endif ?></div><div><?php if ($nextUrl): ?><a class="course-resource-viewer-nav-link" href="<?= mmh_homework_escape($nextUrl) ?>">Next resource <span class="fas fa-arrow-right" aria-hidden="true"></span></a><?php else: ?><a class="course-resource-viewer-nav-link" href="<?= mmh_homework_escape($courseUrl) ?>">Continue learning <span class="fas fa-arrow-right" aria-hidden="true"></span></a><?php endif ?></div></nav></main><script src="<?= mmh_homework_escape(rtrim((string)$baseUrl,'/').'/resources/js/course-homework.js?v='.$homeworkJsVersion) ?>" defer></script></body></html>
<?php exit;
    }
}
