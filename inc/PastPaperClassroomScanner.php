...
if (!function_exists('mmh_classroom_scan')) {
    function mmh_classroom_scan(mysqli $conn, string $courseId, string $syllabusId, string $baseUrl = ''): array
    {
        ...
        try {
            $topics = mmh_classroom_api_list_all(static fn($page) => $service->courses_topics->listCoursesTopics($courseId, array_filter(['pageSize' => 100, 'pageToken' => $page, 'fields' => 'topic(topicId,name),nextPageToken'], static fn($v) => $v !== '')), 'topic');
            $topicRows = array_map(static fn($topic) => ['id' => (string) ($topic->getTopicId() ?? ''), 'title' => (string) ($topic->getName() ?? '')], $topics);
            $metadata = [];
