...
<body class='dash ds-bg-primary admin-courses-page'>
    <style type="text/css">
        ...
        .admin-course-enrollment-cell { align-items: center; display: flex; flex-direction: column; gap: .25rem; }
        .admin-course-enrollment-link { color: var(--primary); font-size: .72rem; font-weight: 750; text-decoration: none; }
        .admin-course-enrollment-link:hover, .admin-course-enrollment-link:focus-visible { color: var(--text-primary); text-decoration: underline; }
                          ...
                          <div class="admin-course-table-wrap">
                            <table class="admin-course-table" id="coursesTable">
                              ...
                              ?><tr>
                                ...
                                <td><div class="admin-course-enrollment-cell"><form method="post" action="" class="addUserToCourse"><input type="hidden" name="_token" value="<?=admin_course_escape(mmh_auth_csrf_token())?>"><input type="hidden" name="course_id" value="<?=admin_course_escape($courseId)?>"><input type="hidden" name="_method" value="GET"><button class="admin-course-count" title="Quick enroll student" aria-label="Quick enroll student in <?=admin_course_escape($course['course_title'])?>"><?=number_format((int)$course['enrolled_students'])?></button></form><a href="<?=admin_course_escape($courseListBase . '/admin/courses/' . rawurlencode($courseId) . '/students')?>" class="admin-course-enrollment-link">Manage</a></div></td>
