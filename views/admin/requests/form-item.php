...
$timed_exam_paper_source = (string) ($timed_exam_config['paper_source'] ?? (!empty($timed_exam_config['paper_storage_key']) ? 'private_upload' : 'external_link'));
          ...
          <fieldset class='form-fieldset api-mode'>
            ...
            <div class='{$pane_class['timed_exam']}' data-template-pane='timed_exam'>
              ...
              <div class='row'>
                ...
                <div class='col-12 p-2'><label class='form-label' for='timed-exam-paper-url'>Exam Paper Link</label><input id='timed-exam-paper-url' type='url' class='form-control' name='timed_exam_paper_url' value=\"" . htmlspecialchars($timed_exam_paper_url, ENT_QUOTES, 'UTF-8') . "\" placeholder='https://drive.google.com/...'><small class='text-muted'>Google Drive, Google Docs/Sheets/Slides, SharePoint, OneDrive, or a secure HTTPS PDF link.</small><div class='alert alert-warning mt-2 mb-0'>Cloud links are controlled by the provider’s sharing permissions. Use a restricted link where supported and do not publish the link elsewhere.</div>" . ($timed_exam_paper_source === 'private_upload' && !empty($timed_exam_config['paper_original_name']) ? "<small class='d-block text-muted mt-2'>Existing uploaded paper retained for compatibility: " . htmlspecialchars((string) $timed_exam_config['paper_original_name'], ENT_QUOTES, 'UTF-8') . ". Adding a link switches this exam to the external paper.</small>" : '') . "</div>
