...
$start = mmh_parent_report_date($_POST['week_start'] ?? $_GET['week_start'] ?? $week['start']) ?? $week['start'];
$end = mmh_parent_report_date($_POST['week_end'] ?? $_GET['week_end'] ?? $week['end']) ?? $week['end'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!mmh_auth_csrf_valid($_POST['_token'] ?? '')) {
        ...
        try {
            ...
            $report = mmh_parent_report_resolve($conn, $selectedCourseId, $selectedStudentId, $start, $end, $commentWasSubmitted ? $comment : null);
