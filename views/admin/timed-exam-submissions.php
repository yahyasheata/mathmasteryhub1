          ...
          <table class="timed-admin-table">
            ...
            <?php foreach ($rows as $row): ?>
              ...
              <tr>
                ...
                <?php if ($gradeable): ?>
                  <td>
                    <form id="<?= $esc($formId); ?>" method="post" action="<?= $esc($gradeUrl); ?>">
                      <input type="hidden" name="_token" value="<?= $esc(mmh_auth_csrf_token()); ?>">
                      ...
                      <textarea name="feedback" rows="2" placeholder="Feedback"><?= $esc($row['feedback'] ?? ''); ?></textarea>
                    </form>
                  </td>
                  <td>
                    <div class="timed-admin-actions">
                      <button class="btn btn-sm btn-primary" type="submit" form="<?= $esc($formId); ?>" name="grade_action" value="save">Save grade</button>
