...
<main class="classroom-scanner-page">
    <header class="classroom-scanner-header">
        <div><div class="classroom-scanner-eyebrow">Past Papers · Migration preview</div><h1 class="h3 mb-1">Google Classroom Scanner</h1><p class="classroom-scanner-muted mb-0">Read-only scan of the approved Classroom topics. Nothing will be imported yet.</p></div>
    ...
    <section class="classroom-scanner-card"><h2 class="h5 mb-3">Paper candidates</h2><div class="classroom-preview-grid"><?php foreach (($preview['candidates'] ?? []) as $candidate): ?><article class="classroom-candidate"><header><div><h3><?= $escape($candidate['topic']['session'] . ' ' . $candidate['topic']['year'] . ' · ' . $candidate['topic']['variant']) ?></h3><div class="classroom-component"><?= $escape($candidate['paper_number']) ?> · Component <?= $escape($candidate['component']) ?></div></div><span class="classroom-status <?= $candidate['status'] === 'READY' ? 'is-ready' : '' ?>"><?= $escape($candidate['status']) ?></span></header><?php if (!empty($candidate['existing_paper'])): ?><div class="alert alert-warning py-2 px-3 mb-2">Existing paper — no changes will be made.</div><?php endif; ?><?php foreach (($candidate['resources'] ?? []) as $resource): $source = $resource['source'] ?? []; ?><div class="classroom-resource"><strong><?= $escape(ucwords(str_replace('_', ' ', (string) $resource['resource_type']))) ?></strong><small><?= $escape($source['item_title'] ?? 'Classroom item') ?><?= !empty($source['attachment_title']) ? ' · ' . $escape($source['attachment_title']) : '' ?></small></div><?php endforeach; ?><?php if (!empty($candidate['missing_resources'])): ?><div class="classroom-scanner-muted mt-2">Missing: <?= $escape(implode(', ', $candidate['missing_resources'])) ?></div><?php endif; ?><details class="mt-3"><summary>Source trace</summary><div class="classroom-scanner-muted mt-2">Topic: <?= $escape($candidate['topic']['title'] ?? '') ?><br><?php foreach (($candidate['resources'] ?? []) as $resource): $source = $resource['source'] ?? []; ?><div class="mt-2"><strong><?= $escape(ucwords(str_replace('_', ' ', (string) $resource['resource_type']))) ?></strong><br>Item: <?= $escape($source['item_title'] ?? 'Classroom item') ?><br>Type: <?= $escape($source['coursework_type'] === 'coursework_material' ? 'CourseWorkMaterial' : 'CourseWork') ?><br>Item ID: <?= $escape($source['coursework_id'] ?? '') ?><br>Attachment <?= (int) ($source['attachment_index'] ?? 0) ?>: <?= $escape($source['attachment_type'] ?? 'unknown') ?><br>Attachment title: <?= $escape($source['attachment_title'] ?? '') ?><br>Detected paper: <?= $escape(implode(', ', array_map(static fn($paper) => 'Paper ' . $paper, (array) ($source['detected_papers'] ?? [])))) ?><?php if (!empty($source['source_url'])): ?><br>Source URL: <?= $escape($source['source_url']) ?><?php endif; ?><?php if (!empty($source['drive_file_id'])): ?><br>Drive file ID: <?= $escape($source['drive_file_id']) ?><?php endif; ?><?php if (!empty($source['youtube_id'])): ?><br>YouTube ID: <?= $escape($source['youtube_id']) ?><?php endif; ?></div><?php endforeach; ?></div></details></article><?php endforeach; ?></div></section>
    <?php endif; ?>
</main>
</div></div>
</body></html>
