    ...
    if(isset($_POST['_method']) && $_POST['_method'] == 'GET' ){
        ...
        if ( isset($_POST['course_id']) && !empty($_POST['course_id']) ) {
            ...
            if($result){
                ...
                $categories_result = mysqli_query($conn,"SELECT category_id,category_title from categories");
                $categorie_options = "";
                while( $categories_data = mysqli_fetch_assoc($categories_result)){
                    if ($categories_data['category_id'] == $c_course_category ) {
                        $option_select =  "selected";
                        $categorie_options .= "<option value='{$categories_data['category_id']}' $option_select>{$categories_data['category_title']}</option>";

                    }else{
                        $categorie_options .= "<option value='{$categories_data['category_id']}' >{$categories_data['category_title']}</option>";
...
if($_SERVER['REQUEST_METHOD'] == "POST" ){
  if(isset($_POST['_method']) && $_POST['_method'] == 'UPDATE' ){
    ...
    && !empty($_POST['course_title']) && !empty($_POST['course_title_en']) && !empty($_POST['course_price']) && !empty($_POST['course_description']) ) {
        ...
        $old_course_image = mysqli_fetch_assoc(mysqli_query(db(),"SELECT course_image FROM courses"))['course_image'];
