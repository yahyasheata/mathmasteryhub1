...
<div id="app"><div id="body-overlay" onclick="document.getElementById('aside-menu').classList.toggle('active');document.getElementById('body-overlay').classList.toggle('active');"></div><?php include 'layouts/user/aside.php'; ?><main class="font-2">
    ...
    <?php elseif ($summary): ?>
      ...
      <section class="recovery-progress-section recovery-progress-card" aria-labelledby="missing-coursework-title"><div class="recovery-progress-section-head"><h2 id="missing-coursework-title">Missing Coursework</h2></div><?php if (!$missingHomeworkGroups): ?><p class="student-progress-empty">No missing homework in this reporting period.</p><?php else: ?><div class="missing-coursework-list"><?php foreach ($missingHomeworkGroups as $missingGroup): ?><article class="missing-coursework-item"><div><h3><?= $escape('Lecture ' . $missingGroup['section_number'] . ' — ' . $missingGroup['topic_title']) ?></h3><?php foreach ($missingGroup['items'] as $missing): ?><p>Homework not submitted · <?= $escape($missing['title']) ?> · <a href="<?= $escape($resourceUrl($selectedCourseId, $missing['item_id'])) ?>">Open Homework</a></p><?php endforeach; ?></div></article><?php endforeach; ?></div><?php endif; ?></section>
