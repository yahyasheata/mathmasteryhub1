...
if ($selectedBatchId > 0) {
    ...
    // Pre-progressive plans stored shared materials at Version scope. Keep
    // those legacy resources visible without making future progressive-batch
    // content available early.
    if (!array_key_exists('unreleased_batches', (array) ($context['version'] ?? [])) || !$unreleasedBatches) {
        foreach ($resources as $resource) {
            if ((int) ($resource['batch_id'] ?? 0) <= 0) $batchMaterials[(int) $resource['id']] = $resource;
        }
    }
}
        ...
        <?php if (is_array($selectedDay)): ?>
            <section class="revision-selected-day <?= $selectedAvailable ? '' : 'is-locked' ?>" aria-labelledby="selected-day-title">
                ...
                <?php else: ?><div class="revision-requirement-list">
                    <?php $taskIndex = 0; foreach ($selectedRequirements as $requirement): $taskIndex++; $requirementId = (int) ($requirement['id'] ?? 0); $type = strtolower(trim((string) ($requirement['requirement_type'] ?? ''))); $actionable = mmh_revision_requirement_is_actionable($requirement); $complete = $actionable && isset($progress[$requirementId]); $titleForAction = strtolower((string) ($requirement['title'] ?? '')); $openLabel = (strpos($titleForAction, 'video') !== false || strpos($titleForAction, 'recording') !== false) ? 'Watch video' : ((strpos($titleForAction, 'sheet') !== false || strpos($titleForAction, 'pdf') !== false) ? 'Open sheet' : 'Open content'); $typeLabel = $type === 'course_item' ? 'Course content' : ($type === 'checklist' ? 'Task' : ($type === 'upload' ? 'Your answer' : ($type === 'resource' ? 'Resource' : ucwords(str_replace('_', ' ', $type))))); ?>
                        <article class="revision-student-requirement <?= $complete ? 'is-complete' : '' ?>"><div class="revision-task-index" aria-hidden="true"><?= str_pad((string) $taskIndex, 2, '0', STR_PAD_LEFT) ?></div><div class="revision-task-copy"><div class="revision-task-meta"><span class="revision-requirement-type"><?= $esc($typeLabel) ?></span><?php if (!$requirement['is_required']): ?><span class="revision-task-optional">Optional</span><?php endif; ?></div><h3><?= $esc($requirement['title']) ?></h3><?php if (($requirement['description'] ?? '') !== ''): ?><p><?= nl2br($esc($requirement['description'])) ?></p><?php endif; ?></div><div class="revision-requirement-action">
                            <?php if ($type === 'course_item' && trim((string) ($requirement['linked_course_item_id'] ?? '')) !== ''): ?><div class="revision-requirement-actions"><a class="student-dashboard-btn secondary" href="<?= $esc($itemUrl((string) $requirement['linked_course_item_id'], $requirementId)) ?>"><?= $esc($openLabel) ?></a><form method="post" action="<?= $esc($completionUrl . $requirementId . '/completion') ?>"><input type="hidden" name="csrf_token" value="<?= $esc(mmh_auth_csrf_token()) ?>"><input type="hidden" name="action" value="<?= $complete ? 'undo' : 'complete' ?>"><button type="submit" class="student-dashboard-btn <?= $complete ? 'secondary is-complete-action' : 'primary' ?>"><?= $complete ? '✓ Done' : 'Mark done' ?></button></form></div>
                            <?php elseif ($type === 'checklist'): ?><form method="post" action="<?= $esc($completionUrl . $requirementId . '/completion') ?>"><input type="hidden" name="csrf_token" value="<?= $esc(mmh_auth_csrf_token()) ?>"><input type="hidden" name="action" value="<?= $complete ? 'undo' : 'complete' ?>"><button type="submit" class="student-dashboard-btn <?= $complete ? 'secondary is-complete-action' : 'primary' ?>"><?= $complete ? '✓ Done' : 'Mark done' ?></button></form>
                            <?php elseif ($type === 'resource' && !empty($requirement['resource_ids'])): ?><div class="revision-requirement-actions"><div class="revision-resource-links"><?php foreach ((array) $requirement['resource_ids'] as $resourceId): if (!isset($resources[(int) $resourceId])) continue; ?><a class="student-dashboard-btn secondary" href="<?= $esc($resourceUrl((int) $resourceId, $requirementId)) ?>"><?= $esc($resources[(int) $resourceId]['display_name']) ?></a><?php endforeach; ?></div><form method="post" action="<?= $esc($completionUrl . $requirementId . '/completion') ?>"><input type="hidden" name="csrf_token" value="<?= $esc(mmh_auth_csrf_token()) ?>"><input type="hidden" name="action" value="<?= $complete ? 'undo' : 'complete' ?>"><button type="submit" class="student-dashboard-btn <?= $complete ? 'secondary is-complete-action' : 'primary' ?>"><?= $complete ? '✓ Done' : 'Mark done' ?></button></form></div>
                            <?php elseif ($type === 'upload'): ?><?php $submission = mmh_revision_requirement_submission($conn, $assignmentId, (int) $userId, $requirementId); ?><div class="revision-upload-control"><?php if (!empty($requirement['resource_ids'])): ?><div class="revision-resource-links"><?php foreach ((array) $requirement['resource_ids'] as $resourceId): if (!isset($resources[(int) $resourceId])) continue; ?><a class="student-dashboard-btn secondary" href="<?= $esc($resourceUrl((int) $resourceId, $requirementId)) ?>"><?= $esc($resources[(int) $resourceId]['display_name']) ?></a><?php endforeach; ?></div><?php endif; ?><form method="post" enctype="multipart/form-data" action="<?= $esc($base . '/user/revision-plan/' . $assignmentId . '/requirement/' . $requirementId . '/upload') ?>"><input type="hidden" name="csrf_token" value="<?= $esc(mmh_auth_csrf_token()) ?>"><label class="revision-upload-label"><span><?= $submission ? 'Replace answer PDF' : 'Your answer' ?></span><input type="file" name="revision_files[]" accept="application/pdf,.pdf" multiple required></label><button type="submit" class="student-dashboard-btn <?= $submission ? 'secondary' : 'primary' ?>"><?= $submission ? 'Replace upload' : 'Submit answer' ?></button><?php if ($submission): ?><small class="revision-upload-status"><?= count((array) ($submission['files'] ?? [])) ?> PDF<?= count((array) ($submission['files'] ?? [])) === 1 ? '' : 's' ?> uploaded · <?= $esc(date('j M Y', strtotime((string) $submission['submitted_at']))) ?></small><?php endif; ?></form></div><?php else: ?><span class="revision-later-note">This task has no completion control yet.</span><?php endif; ?>
                        </div></article>
                    <?php endforeach; ?>
                </div><?php endif; ?>
            </section>
        <?php endif; ?>
        <?php if ($selectedAvailable && $batchMaterials): ?><section class="revision-batch-materials revision-batch-materials-top" aria-label="Batch resources"><div><span class="revision-student-card-kicker">Batch resources</span></div><div class="revision-material-list"><?php foreach ($batchMaterials as $material): $materialId = (int) $material['id']; $materialRequirementId = (int) ($batchMaterialRequirementIds[$materialId] ?? 0); $materialType = strtolower((string) ($material['resource_type'] ?? '')); $materialLabel = $materialType === 'uploaded_pdf' ? 'PDF' : ($materialType === 'course_item' ? 'Course content' : ($materialType === 'external_link' ? 'Link' : 'Resource')); $materialAction = $materialType === 'uploaded_pdf' ? 'Open PDF' : ($materialType === 'course_item' ? 'Open content' : ($materialType === 'external_link' ? 'Open link' : 'Open')); ?><a class="revision-material-row" href="<?= $esc($resourceUrl($materialId, $materialRequirementId)) ?>"><span class="revision-material-copy"><strong><?= $esc($material['display_name']) ?></strong><small><?= $esc($materialLabel) ?></small></span><span class="revision-material-open"><?= $esc($materialAction) ?><span aria-hidden="true">→</span></span></a><?php endforeach; ?></div></section><?php endif; ?>
    </section>
</main></div>
<script>
(function(){
    ...
    document.querySelectorAll('.revision-upload-control form[enctype="multipart/form-data"]').forEach(function(form){
        ...
        zone.innerHTML = '<strong>Your answer</strong><span>Add one or more PDF files.</span><button type="button" class="student-dashboard-btn secondary revision-upload-choose" aria-label="Choose PDF files"><span aria-hidden="true">＋</span> Add PDFs</button><div class="revision-upload-selected" aria-live="polite"></div>';
