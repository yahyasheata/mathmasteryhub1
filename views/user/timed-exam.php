  ...
  <section class="timed-exam-card" aria-labelledby="timed-exam-instructions-title">
    ...
    <?php elseif ($activeState): ?>
      ...
      <section class="timed-exam-upload-card" aria-labelledby="timed-exam-answer-title">
        ...
        <?php if ($timedExamPreview): ?><div class="timed-exam-preview-controls"><p class="timed-exam-message">Preview only — choose a file to inspect the upload state. The file stays in your browser and is never uploaded.</p><div class="timed-exam-actions"><label class="timed-exam-choose" for="answer-file"><span class="fas fa-folder-open" aria-hidden="true"></span> Choose File</label><input id="answer-file" class="timed-exam-file-input" type="file" accept="<?= $esc($acceptedExtensions); ?>" aria-describedby="answer-file-help"><button class="course-btn course-btn-primary" type="button" data-preview-submit disabled><span class="fas fa-paper-plane" aria-hidden="true"></span> Submit Exam</button></div><div id="answer-file-help" class="timed-exam-file-help">Accepted: <?= $esc($acceptedTypes); ?> · Maximum <?= (int) $maxFileSizeMb; ?> MB</div><p class="timed-exam-message" data-preview-file role="status"></p><p class="timed-exam-message" data-preview-message role="status"></p></div>
    ...
    <?php elseif (in_array($stateKey, ['submitted', 'auto_submitted', 'graded'], true)): ?>
      ...
      <?php if ($stateKey === 'graded' && $resultReleased): ?><div class="timed-exam-status"><strong>Grade: <?= $esc($attempt['grade'] ?? 'Not entered'); ?><?= $exam['max_marks'] !== null ? ' / ' . $esc($exam['max_marks']) : ''; ?></strong><?php if (($attempt['feedback'] ?? '') !== ''): ?><span><?= $esc($attempt['feedback']); ?></span><?php endif; ?></div><?php elseif ($stateKey === 'graded'): ?><p class="timed-exam-message">Your answer has been graded. The result has not been released yet.</p><?php endif; ?>
...
(function () {
  ...
  if (previewMode) {
    ...
    if (previewSubmit) previewSubmit.addEventListener('click', function () {
      if (previewMessage) previewMessage.textContent = 'Preview mode — no submission was created.';
    });
    return;
  }
