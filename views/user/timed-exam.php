...
$attemptNumber = max(0, (int) ($attempt['attempt_number'] ?? 0));
$maxAttempts = max(1, (int) ($exam['max_attempts'] ?? 1));
$attemptSummary = $attemptNumber > 0 ? 'Attempt ' . $attemptNumber . ' of ' . $maxAttempts : $maxAttempts . ' attempt' . ($maxAttempts === 1 ? '' : 's');
...
<main class="timed-exam-shell">
  ...
  <section class="timed-exam-card" aria-labelledby="timed-exam-instructions-title">
    ...
    <?php elseif (in_array($stateKey, ['submitted', 'auto_submitted', 'graded'], true)): ?>
      ...
      <?php if ($stateKey === 'graded' && (empty($exam['results_release_at_utc']) || strtotime((string) $exam['results_release_at_utc']) <= time())): ?><div class="timed-exam-status"><strong>Grade: <?= $esc($attempt['grade'] ?? 'Not entered'); ?><?= $exam['max_marks'] !== null ? ' / ' . $esc($exam['max_marks']) : ''; ?></strong><?php if (($attempt['feedback'] ?? '') !== ''): ?><span><?= $esc($attempt['feedback']); ?></span><?php endif; ?></div><?php endif; ?>
    <?php else: ?>
